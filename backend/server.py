...
async def export_dossier_pdf(request: Request, project_id: Optional[str] = None):
    ...
    if len(projects) > 1:
        ...
        pdf.cell(0, 7, f"  Total Approved Trees: {total_trees:,}", ln=True)
        pdf.cell(0, 7, f"  Estimated tCO2e (all projects): {total_credits:.4f}", ln=True)
    ...
    for proj in projects:
        ...
        pdf.cell(0, 12, f"Project: {proj.get('name', 'N/A')}", ln=True, fill=True)
        pdf.ln(3)
        ...
        pdf.cell(0, 7, f"  Status        : {proj.get('status', 'active').upper()}", ln=True)
        pdf.cell(0, 7, f"  Created       : {proj.get('created_at', 'N/A')[:10]}", ln=True)
        if proj.get("description"):
            pdf.set_font("Helvetica", "", 10)
            pdf.multi_cell(0, 6, f"  Description   : {proj['description']}")
        pdf.ln(4)
        ...
        for s in proj.get("species_list", []):
            pdf.cell(0, 7, f"  - {s.get('name', '').capitalize()}: {s.get('growth_rate', 'medium')} growth", ln=True)
        ...
        pdf.multi_cell(0, 6, "  DISCLAIMER: All values are estimates. Final issuance subject to third-party verification and registry approval.")
        pdf.ln(4)
        ...
        pdf.cell(0, 7, f"  Estimated payout (INR) : {p_payout:,.2f}", ln=True)
        pdf.ln(4)

        # 7. Farmer Summary Table (first 15)
        pdf.set_font("Helvetica", "B", 13)
        pdf.cell(0, 9, "7. Farmer Roster (up to 15 shown)", ln=True)
        ...
        for f in proj_farmers[:15]:
            pdf.cell(55, 6, str(f.get("name", ""))[:30], border=1)
            pdf.cell(32, 6, str(f.get("phone", "")), border=1)
            pdf.cell(22, 6, str(f.get("land_type", "")), border=1)
