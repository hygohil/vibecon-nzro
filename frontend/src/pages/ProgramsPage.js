...
import { Plus, TreePine, MapPin, Trash2, Eye, X } from 'lucide-react';
...
export default function ProgramsPage() {
  ...
  const deleteProgram = async (id) => {
    try {
      const res = await fetch(`${API}/programs/${id}`, { method: 'DELETE', credentials: 'include' });
      if (res.ok) { toast.success('Program deleted'); fetchPrograms(); }
  ...
  return (
    <div data-testid="programs-page" className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-[#1F2937] tracking-tight" style={{ fontFamily: 'Manrope, sans-serif' }}>Programs</h1>
          <p className="text-[#6B7280] mt-1">Manage your tree plantation programs</p>
        </div>
        <Button onClick={() => setShowCreate(true)} data-testid="create-program-btn" className="bg-[#1A4D2E] text-white hover:bg-[#143C24] shadow-sm font-medium px-5 py-2.5 rounded-lg transition-all active:scale-95">
      ...
      {programs.length === 0 ? (
        <Card className="bg-white border border-gray-100 rounded-xl shadow-sm">
          <CardContent className="py-16 text-center">
            ...
            <p className="text-[#6B7280]">No programs yet. Create your first tree plantation program.</p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {programs.map((p) => (
            <Card key={p.program_id} data-testid={`program-card-${p.program_id}`} className="bg-white border border-gray-100 rounded-xl shadow-[0_2px_8px_rgba(0,0,0,0.04)] hover:shadow-md transition-shadow duration-300">
              <CardContent className="p-5">
                ...
                <div className="grid grid-cols-3 gap-2 mb-4">
                  ...
                  <div className="text-center p-2 bg-gray-50 rounded-lg">
                    <p className="text-lg font-bold text-[#B45309]" style={{ fontFamily: 'Manrope, sans-serif' }}>{p.claims_count}</p>
                ...
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => setShowDetail(p)} data-testid={`view-program-${p.program_id}`} className="flex-1 text-xs border-[#1A4D2E]/20 text-[#1A4D2E] hover:bg-emerald-50">
                    <Eye className="w-3 h-3 mr-1" /> View
                  </Button>
                  <Button variant="outline" size="sm" onClick={() => deleteProgram(p.program_id)} data-testid={`delete-program-${p.program_id}`} className="text-xs border-red-200 text-red-500 hover:bg-red-50">
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto" data-testid="create-program-dialog">
          <DialogHeader>
            <DialogTitle style={{ fontFamily: 'Manrope, sans-serif' }}>Create Tree Program</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div>
              ...
              <Textarea data-testid="program-desc-input" value={form.description} onChange={e => setForm({...form, description: e.target.value})} placeholder="Brief description" className="mt-1" rows={2} />
            </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreate} data-testid="submit-program-btn" className="bg-[#1A4D2E] text-white hover:bg-[#143C24]">Create Program</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!showDetail} onOpenChange={() => setShowDetail(null)}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto" data-testid="program-detail-dialog">
          <DialogHeader>
          ...
          {showDetail && (
            <div className="space-y-4 text-sm">
              ...
              <div className="p-3 bg-gray-50 rounded-lg font-mono text-xs text-[#6B7280]" data-testid="program-id-display">
                Program ID: {showDetail.program_id}
              </div>
            </div>
          )}
        </DialogContent>
